...
L<Protocol::WebSocket> module and many available event loops, and even a simple
echo server using only L<IO::Socket::INET> and L<IO::Poll>.
...
=head2 Ready to go WebSocket servers and Web Frameworks
...
=item Nov 2010 ReAnimator 0.0001
...
L<http://github.com/vti/reanimator>.

=item Nov 2010 L<Mojolicious> 0.999941
...
=item Nov 2010 L<Dancer> 1.2000

L<Dancer>'s WebSocket support is experimental, but there is a chat plugin
available on Github L<https://github.com/franckcuny/dancer-chat>. It uses
L<Web::Hippie> middleware.

=item Oct 2010 L<Plack::Middleware::WebSocket>

This L<Plack> middleware is not CPAN, but available from GitHub
L<https://github.com/motemen/Plack-Middleware-WebSocket>. Below is a simplified
...
=item Jun 2010 AnyEvent::HTTP::Server L<https://github.com/Mons/AnyEvent-HTTP-Server>

This server supports drafts 75 and 76 and provides all the power that
L<AnyEvent> has, including SSL, async DNS etc.

Some author's modules are required to run this application, like L<uni::perl>,
L<accessors::fast>, C<HTTP::Easy> L<https://github.com/Mons/HTTP-Easy>.

Example below is greatly simplified. Full source can be found in repository
L<https://github.com/Mons/AnyEvent-HTTP-Server/blob/master/ex/ws-echo/server.pl>.
...
=head2 Extendable async servers and web frameworks

=over 4

=item Oct 2010 L<Twiggy> 0.1008

I coudn't make draft 76 to work with L<Twiggy>, but there is a draft 75 WebSocket
example in the distribution
L<http://cpansearch.perl.org/src/MIYAGAWA/Twiggy-0.1008/eg/chat-websocket/chat.psgi>.

=item Apr 2010 L<Tatsumaki> 0.1010

There is a development branch called C<websocket>, and there is an example of
websocket chat, but unfortunately it supports only draft 75.

You can check it out on GitHub
L<https://github.com/miyagawa/Tatsumaki/commit/38fa7f5d215dd683f95949cc8fdad8565a13b954>.

=back

Actually theoretically any async web framework running on L<Plack> with
C<streaming> and C<nonblocking> support can be a WebSocket server. But
practically I coudn't find one.

=head2 Extendable with L<Protocol::WebSocket> low level tcp event loops

First of all what L<Protocol::WebSocket> is? This is a set of modules for
...
You can read more about L<Protocol::WebSocket> in my previous post
L<http://showmetheco.de/articles/2010/11/parsing-websocket-messages-with-protocol-websocket.html>.

=over 4

=item Nov 2010 L<EventReactor> 0.0001

ReAnimator itself uses L<EventReactor> as event loop, which obviously makes it
...
L<http://github.com/vti/event_reactor>.

=item Nov 2010 L<POE::Component::Server::TCP> 1.294

L<POE> is probably the most well known event loop for Perl. It has a huge
collection of modules for any needs. This implementation uses
L<POE::Component::Server::TCP> which provides a simple C<tcp> server.
...
=item Oct 2010 L<AnyEvent> 5.28

L<AnyEvent> is probably the most popular event loop in Perl now, because it
...
Other nice chat example built on top of L<AnyEvent> you can find on GitHub
L<https://github.com/typester/anyevent-websocket-demo/blob/master/chat.pl>.

=item Oct 2010 L<Reflex> 0.085

Object oriented events using L<Moose> and L<POE> to support many event loops and many syntaxes.
...
=item Jul 2010 L<Net::Server::Multiplex> 0.99

L<Net::Server::Multiplex> is like L<IO::Multiplex>, but has a more high-level
...
=item Jul 2009 L<IO::Event> 0.704
...
=item Sep 2008 L<IO::Multiplex> 1.10
...
=item Mar 2003 L<Net::Socket::NonBlock> 0.15
...
=head2 Using just L<IO::Socket::INET> and L<IO::Poll>

A real low level stuff for creating a WebSocket server in Perl. We use
L<IO::Poll> for multiplexing, but any other multiplexer can be used of course.
