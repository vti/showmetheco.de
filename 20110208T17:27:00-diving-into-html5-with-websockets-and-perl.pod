...
particular event occures. So C<onopen> will be called when a connection with
WebSocket server is established, C<onclose> when it is closed, C<onmessage> when
a message is received. There is also C<onerror> callback, but it is not shown
for simplicity. Every time when you want to send a message call C<send>
...
using HTTP/1.1 C<Upgrade> header. Here is an example what happens behind the
scene:
...
L<http://tools.ietf.org/id/draft-ietf-hybi-thewebsocketprotocol-00>) handshake
was changed adding C<Sec-*> fields and challenges for various security reasons.
...
L<http://tools.ietf.org/id/draft-ietf-hybi-thewebsocketprotocol-02> already has
a much more complicated frame structure, current browsers don't support it
though), but the client API should stay the same.

Other C<HTTP> headers passed along are mostly ignored, but there is one that is
definitely important, and that is C<Cookie>. With use of cookies such important
...
client will be aware of this when connecting via C<wss://> instead of C<ws://>.
...
=head2 Nonwebsocket Browser Support
...
There is a Socket.IO for Perl implemented on top of L<Mojolicious>
L<https://github.com/xantus/ignite> (long polling and WebSockets).

=head2 WebSocket Issues
...
=head3 Reverse Proxy Support

The Draft #76 introduced a challenge that is sent as a body to the
WebSocket client. The problem is that it doesn't have a C<Content-Length> header
...
including adding a C<Content-Length: 8> or C<Connection: close> headers, using a
C<POST> method and others.

=head3 Framing

The C<\x00..\xff> framing was considered not the best solution, since using just
...
=head2 Using WebSockets In A Real Life
...
=head3 WebSocket Server Requirements

The obvious requirements are:

=over

=item * It must know how to parse WebSocket handshake and frames

=item * It must be asynchronous and nonblocking

=item * It must be able to hold many persistent connections
...
L<Mojolicious>, which has a WebSocket client in addition, some work is being
done in L<Dancer>, L<Tatsumaki>. Standalone WebSocket servers include those
built on top of various event loops, check out L<Net::Async::WebSocket>,
L<AnyEvent::HTTP::Server> and also L<Plack> middleware
L<https://github.com/motemen/Plack-Middleware-WebSocket>. More info on how to
build a WebSocket server in Perl you can find in the following article
L<http://showmetheco.de/articles/2010/11/timtow-to-build-a-websocket-server-in-perl>.
...
handshake (or by using L<Protocol::WebSocket>) and doing nonblocking in Perl.
...
=head2 WebSocket Demo Applications

=head3 Configuration And Installation

Examples are accessible online from my GitHub page and usually don't require
third-party modules installation. The only exception is L<JSON>, but I am pretty
sure it is already installed on every Perl developer's machine.

=head3 Simple Echo Server
...
a git repository you will find C<examples/> directory there (you can find more
examples of WebSocket echo servers in L<Protocol::WebSocket> distribution or on
GitHub L<http://github.com/vti/protocol-websocket> in L<examples/> directory).
...
And open a static C<index.html> page, that lies in the C<examples/public/>
directory, in your browser.

If you get a C<Disconnected> message it means that either your browser does not
support WebSockets or there is a server problem. The server can be debugged by
setting up C<EVENT_REACTOR_DEBUG=1> environmental variable.

On success you will get an C<input> field and a C<submit> button. If you send
...
Inside we have a static C<index.html> page that connects with a help of
JavaScript to the WebSocket server via API described earlier and there is an
C<echo> Perl script with a few lines of code:
...
This code should remind you the code in JavaScript, here we have an C<on_accept>
hook that is run when a new connection is accepted, C<on_message> hook that is
run when a new message (already correctly parsed and decoded to UTF-8) arrives.
A C<send_message> method is used to send a message to the browser. In
this simple example we just send it back without any change.

To eliminate some confusions C<on_accept> hook is called not when a normal
...
it C<on_connect> because that would make a reader think that a server
...
Of course the best way to administrate a server is via C<ssh>. You get a full
featured terminal and you don't even notice that you are working on a remote
side. But sometimes you either don't have a C<ssh> client installed on your
machine or there is no C<ssh> access to your server.
...
colors, understand commands like C<CTRL-C>, C<CTRL-D> and so on.

For the pseudo terminal we will use L<IO::Pty>, for the terminal emulation and
characters handling L<Term::VT102>.
...
=item Create a new pty object making it a slave to a normal tty

=item Reopen C<STDIN>, C<STDOUT> and C<STDERR> so they point to our pty object

=item Exec a command like C</bin/sh> to start a shell inside a new terminal

=item Register callbacks that are invoked when a new data arrives

=back

This is all for the server side. For simplicity I put everything in one class
and called it C<Terminal.pm>, you can find the source code on GitHub
L<https://github.com/vti/showmetheshell/blob/master/lib/Terminal.pm>. Now
...
framework this can be greatly simplified (e.g. C<JQuery>). Another solution is
to create a virtual keyboard.

Since the server sends which row has changed, there are no issues displaying it
with C<JavaScript>. I've created 24 C<span> rows with unique ids and used this
function:
...
=head3 Important Enhancement

The obvious enhancement is to use SSL of course. You don't want to run a C<ssh>
session via a not protected connection. And since WebSockets support SSL it's as
easy as replacing C<ws://> with C<wss://> (evidently, the server must support
SSL too).

=head3 Demo And Source code

The full source code of this example is available at
L<http://github.com/vti/showmetheshell>. The demo video is also available at
L<http://vimeo.com/13681309>.

=head3 VNC

If you ever wanted to connect to other PC and be able actually see the desktop
and use your keyboard and mouse (not via C<ssh>) you probably know what VNC is.
...
in a browser. While RFB operates on pixels we can use HTML5 C<canvas> that has an
...
The full source code of this example is available at
L<http://github.com/vti/showmethedesktop>. To run this application you need a
