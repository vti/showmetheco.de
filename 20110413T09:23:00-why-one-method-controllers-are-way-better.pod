...
You can't map special Perl methods like C<new>, C<DESTROY> etc.
...
default CRUD methods like C<add>, C<remove> etc. But what if you don't need
C<add> method? Should you overwrite it as an empty sub? You can use roles
...
method C<run>. Then you can map whatever you want to just a single class.
...
=item Code is cleaner

=item Easy to inherit, decorate and combine

=item Easy to test just one action

=item No need to worry about method names
...
=item More classes (but they are simple)

=item Maybe an overkill for simple apps
