...
The first naive approach is to put everything into one hash and call it C<stash>.
...
Basically C<Registry> is a singleton (globally accessible) that holds objects of
...
=head2 Dependency Injection Container

Let's try another pattern called C<Dependency Injection Container>, which is a
local C<Registry> with C<Dependency Injection> capabilities. C<Dependency Injection>
...
This may look like more work, but this way you get rid of a static C<Engine>
dependency and can pass a modified object with the same interface without
modifying C<Car> class.
...
So the C<Dependency Injection Container> is a container that is populated with
...
Below is an example of a C<Dependency Injection Container>.

Let's call an object inside of a container a C<service>. We can pass already
...
To get a service from a container let's use C<get_service> method. The objects
...
C<create_service> when you need a fresh service every time):
...
can be passed as normal arguments to the constructor (C<Constructor Injection>),
...
=head3 Setter Injection

Sometimes you don't want to pass dependencies to constructor, but instead call a
setter (C<Setter Injection>). This can be implemented like this:
...
After MyService creation method C<set_other_service> will be called with
C<other_service> as a parameter:
...
L<Bread::Board> (see also L<IOC>, it's not maintained anymore but you can find
interesting links for further reading), L<Peco::Container> and others with many
