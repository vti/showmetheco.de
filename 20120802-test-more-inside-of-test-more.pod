...
While working on L<http://perltuts.com> I had to write a test runner what would
test tutorial exercises. It would create a Perl package that uses L<Test::More>
...
with a L<Test::Class> (uses L<Test::Builder> internally). Needless to say it
didn't work, because L<Test::More> is an evil singleton! I had to deploy soon so
...
Where C<$code> is initial source code of an exercise, C<$eval_result> is
a result of evaling it and C<$test> is the actual test.

Now let's come back to the pain I had with a TestRunner. In order to isolate
L<Test::More> to its own environment the following steps were made:
...
Will all the above manipulations plus a manual C<stderr> capturing (yes, I know
about L<Capture::Tiny> but it didn't work, maybe because L<Test::More> and
