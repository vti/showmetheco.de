...
L<Design by Contract|http://en.wikipedia.org/wiki/Design_by_contract> is a
...
=item * Checking of input and output values, and thrown exceptions

=item * Conformance to Liskov (L in SOLID) substitution principle
...
=item * Contract definition should leave class still readable

=item * No CPU intensive checking

=item * Could be switched off

=item * Must be inheritable

=item * Denies contract changes in child classes

=item * Must throw understandable errors

=item * As little magic as possible

=item * Should look like Perl
...
This doesn't look very much like Perl to me. Especially the C<ctor> function
...
For now the code is only on GitHub L<http://github.com/vti/attribute-contract>.
Read more there to get familiar with the API.

=head2 When (not) to use
